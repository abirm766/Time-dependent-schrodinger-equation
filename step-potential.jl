...
for t=1:150
    ...
    PyPlot.plot(x, V, label="V"); # Plot the step potential
    PyPlot.legend();
    PyPlot.tight_layout();
    
    fname = "tdse_step_$(@sprintf("%03d", t)).png";
...
save("tdse_step.gif", cat(images..., dims=3), fps=100);   # combining all frame to make gif
for filename in filenames
    if isfile(filename)
